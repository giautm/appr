...
module.exports = function postDeploy() {
  const expUrl = `https://expo.io/@${config.expUsername}/${utils.readAppJSON().slug}?release-channel=${utils.getExpChannelName()}`;
  const expUrlForQRCode = `https://exp.host/@${config.expUsername}/${utils.readAppJSON().slug}?release-channel=${utils.getExpChannelName()}`;
